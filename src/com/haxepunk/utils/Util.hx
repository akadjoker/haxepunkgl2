...
public static inline function createMtrix2D(x:Float, y:Float, scale:Float = 1, rotation:Float = 0):Matrix3D 
   {
	  ...
	  var mat:Matrix3D = new Matrix3D(Vector.ofArray ([
        c*scale,  -s*scale, 0,  0,
        s*scale,  c*scale, 0,  0,
        0,        0,        1,  0,
       x,        y,        0,  1
	...
	public static inline function rect_transform (rect:Rectangle, m:Matrix):Rectangle {
		...
		r.setTo (tx0 + m.tx, ty0 + m.ty, tx1 - tx0, ty1 - ty0);
		return r;
