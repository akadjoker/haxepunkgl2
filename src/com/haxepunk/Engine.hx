...
class Engine extends Sprite
{
	...
	private function renderGL (event:RenderEvent):Void 
	{
		HXP.renderer= cast event.renderer;
		HXP.gl = HXP.renderer.gl;
		glInitialize(HXP.gl);
...
EnterFrame();
	...
	private function render ():Void 
	{
	    ...
	    HXP.gl.clearColor(0,0,0.4, 1);
        HXP.gl.clear(HXP.gl.COLOR_BUFFER_BIT | HXP.gl.DEPTH_BUFFER_BIT);
...
private function renderView(rect:Rectangle):Void 
{
	...
	private function EnterFrame()
	{
		...
		if (paused) 
		{
		_frameLast = _time; // continue updating frame timer
		doRender = false;
		}
		else 
		{
