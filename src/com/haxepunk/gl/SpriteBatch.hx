...
private static var _scratchQuadArray :Float32Array = null;
  ...
  if (_scratchQuadArray == null) {
    ...
    vertexBuffer = HXP.gl.createBuffer();
    indexBuffer = HXP.gl.createBuffer();


    //upload the index data
    HXP.gl.bindBuffer(HXP.gl.ELEMENT_ARRAY_BUFFER, indexBuffer);
    HXP.gl.bufferData(HXP.gl.ELEMENT_ARRAY_BUFFER,  new Int16Array(indices), HXP.gl.STATIC_DRAW);

	indices = null;
    HXP.gl.bindBuffer(HXP.gl.ARRAY_BUFFER, vertexBuffer);
    HXP.gl.bufferData(HXP.gl.ARRAY_BUFFER,  new Float32Array(numVerts), HXP.gl.DYNAMIC_DRAW);
	//HXP.gl.bufferData(HXP.gl.ARRAY_BUFFER, vertices, HXP.gl.DYNAMIC_DRAW);
	//HXP.gl.bufferData(HXP.gl.ARRAY_BUFFER, vertices, HXP.gl.STREAM_DRAW);
...
vertices[index++] = r;vertices[index++] = g;vertices[index++] = b;vertices[index++] = a;		
	...
	private   function start()
    {
     ...
     HXP.gl.vertexAttribPointer(Filter.vertexAttribute, 3, HXP.gl.FLOAT, false, vertexStrideSize, 0);
     HXP.gl.vertexAttribPointer(Filter.texCoordAttribute  , 2, HXP.gl.FLOAT, false, vertexStrideSize, 3 * 4);
     HXP.gl.vertexAttribPointer(Filter.colorAttribute, 4, HXP.gl.FLOAT, false, vertexStrideSize, (3+2) * 4);
...
private   function flush():Void
{
  ...
  //  HXP.gl.bufferSubData(HXP.gl.ARRAY_BUFFER, 0, new Float32Array(vertices));
  //  HXP.gl.bufferData(HXP.gl.ARRAY_BUFFER, vertices, HXP.gl.STREAM_DRAW);
	HXP.gl.bufferSubData(HXP.gl.ARRAY_BUFFER, 0, vertices);
	HXP.gl.bindBuffer(HXP.gl.ELEMENT_ARRAY_BUFFER, this.indexBuffer);
    HXP.gl.drawElements(HXP.gl.TRIANGLES, currentBatchSize * 6, HXP.gl.UNSIGNED_SHORT, 0);
...
{
	this.vertices = null;
	HXP.gl.deleteBuffer(indexBuffer);
